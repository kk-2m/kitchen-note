                        ...
                        @empty (old('ingredient') && old('stock'))
                            <div class="sotck-item px-4 rounded-lg border border-gray-300" id="ingredient-item">
                                <div class="px-8 py-6">
                                    ...
                                    @else
                                        <div class="stock_category px-8">
                                            <label for="category_readonly" class="font-semibold text-gray-400">カテゴリを選択：</label>
                                            <input type="text" class="bold-gray-200 text-gray-400" id="category_readonly" value="{{ $stock->ingredient->ingredient_category->category }}" readonly/>
                                            <input type="hidden" name="ingredient[ingredient_category_id]" value="{{  $stock->ingredient->ingredient_category_id }}"/>
                                        </div>
                                        <div class="stock_name pt-6 px-8">
                                            <label for="name" class="font-semibold text-gray-400">材料名：</label>
                                            <input type="text" class="bold-gray-200 text-gray-400" name="ingredient[name]" id="name" value="{{ old('ingredient.name', $stock->ingredient->name) }}" readonly/>
                                        ...
                                        <div class='flex pt-6 px-8'>
                                            <div class="stock_quantity">
                                                <label for="quantity" class="font-semibold text-gray-400">量：</label>
                                                <input type="number" class="bold-gray-200 text-gray-400" name="stock[quantity]" id="quantity" placeholder="値を入力" value="{{ old('stock.quantity', $stock->quantity) }}" min="0" step="0.01" max="99999999" readonly/>
                                            </div>
                                            <div class="unit pl-2">
                                                <input type="text" class="bold-gray-200 text-gray-400" value="{{ $stock->unit->name }}" readonly/>
                                                <input type="hidden" name="stock[unit_id]" value="{{  $stock->unit_id }}"/>
                                            </div>
                                        </div>
                                    @endif
                                    <!-- 賞味・消費期限を入力 -->
